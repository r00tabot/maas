...
class TestBootResourcesApi(ApiCommonTests):
    ...
    async def test_list_boot_resources_200_has_items(
        self,
        services_mock: ServiceCollectionV3,
        mocked_api_client_user: AsyncClient,
    ) -> None:
        services_mock.boot_resources = Mock(BootResourceService)
        services_mock.boot_resources.get_many.return_value = [
        ...
        assert len(boot_resource_list_response.items) == 2

    async def test_list_boot_resources_200_no_items(
        self,
        services_mock: ServiceCollectionV3,
        mocked_api_client_user: AsyncClient,
    ) -> None:
    ...
    async def test_list_boot_resources_200_with_type_has_items(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}?type=uploaded"
        )
        ...
        services_mock.boot_resources.get_many.assert_called_once_with(
            query=QuerySpec(

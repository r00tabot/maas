...
class TestNOSInstallerApi(ApiCommonTests):
    ...
    async def test_serve_onie_image_success(
        self,
        services_mock: ServiceCollectionV3,
        mocked_api_client: AsyncClient,
    ) -> None:
        """Test successful ONIE image serving."""
        from io import BytesIO

        onie_headers = {
            "onie-eth-addr": TEST_MAC_ADDRESS,
            "onie-vendor-id": "mellanox",
            "onie-arch": "x86_64",
        }

        onie_image = BootResource(
            id=TEST_BOOT_RESOURCE_ID,
            rtype=BootResourceType.UPLOADED,
        ...
        boot_resource_file = BootResourceFile(
            id=1,
            filename="installer.bin",
            filetype=BootResourceFileType.SELF_EXTRACTING,
            sha256="a" * 64,
            size=1024 * 1024,
            filename_on_disk="mellanox-onie-installer.bin",
            resource_set_id=TEST_RESOURCE_SET_ID,
            created=utcnow(),
            updated=utcnow(),
            extra={},
        ...
        services_mock.switches = AsyncMock()
        services_mock.switches.check_installer_for_switch.return_value = (
            TEST_BOOT_RESOURCE_ID
        )
        services_mock.boot_resources = AsyncMock()
        services_mock.boot_resources.get_by_id.return_value = onie_image
        services_mock.boot_resource_sets = AsyncMock()
        services_mock.boot_resource_sets.get_latest_complete_set_for_boot_resource.return_value = boot_resource_set
        services_mock.boot_resource_files = AsyncMock()
        services_mock.boot_resource_files.get_files_in_resource_set.return_value = [
            boot_resource_file
        ]

        mock_file_content = b"ONIE installer binary content"
        mock_file = BytesIO(mock_file_content)

        with patch(
            "maasapiserver.v3.api.public.handlers.nos.open",
            return_value=mock_file,
        ):
            response = await mocked_api_client.get(
                self.BASE_PATH,
                headers=onie_headers,
            )

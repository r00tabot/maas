...
class TestBootSourceSelectionsApi(ApiCommonTests):
    ...
    def user_endpoints(self) -> list[Endpoint]:
        return [
            ...
            Endpoint(method="GET", path=f"{self.BASE_PATH}/monitor"),
            Endpoint(method="GET", path=f"{self.BASE_PATH}/monitor/1"),
        ]
    ...
    async def test_list_selection_status_one_page(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1&id={TEST_BOOTSOURCESELECTION.id}"
        )
        assert response.status_code == 200
    ...
    async def test_list_selection_status_another_page(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1&id={TEST_BOOTSOURCESELECTION.id}&id=2"
        )
        assert response.status_code == 200
        list_response = BootSourceSelectionStatusListResponse(
            **response.json()
        ...
        assert list_response.next == (
            f"{self.BASE_PATH}/monitor?page=2&size=1&id={TEST_BOOTSOURCESELECTION.id}&id=2"
        )
    ...
    async def test_list_selection_status_empty(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1"
        )
        assert response.status_code == 200
        list_response = BootSourceSelectionStatusListResponse(
            **response.json()
    ...
    async def test_list_selection_status_filter_by_ids(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1&id=1&id=2"
        )
        assert response.status_code == 200
    ...
    async def test_list_selection_status_only_selected(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1&selected=true"
        )
        assert response.status_code == 200
    ...
    async def test_list_selection_status_filter_selected_only_and_id(
        ...
        response = await mocked_api_client_user.get(
            f"{self.BASE_PATH}/monitor?page=1&size=1&selected=true&id=1"
        )
        assert response.status_code == 200

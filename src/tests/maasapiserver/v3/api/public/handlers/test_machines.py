...
class TestMachinesApi(ApiCommonTests):
    ...
    @pytest.fixture
    def endpoints_without_authorization(self) -> list[Endpoint]:
        return [
            Endpoint(
                method="GET",
                path=self.BASE_PATH,
    ...
    async def test_list_other_page(
        ...
        services_mock.openfga_tuples.get_client.return_value = AsyncMock(
            OpenFGAClient
        )
        services_mock.openfga_tuples.get_client.list_pools_with_view_machines_access.return_value = []
        services_mock.openfga_tuples.get_client.list_pools_with_view_available_machines_access.return_value = [
            TEST_MACHINE_2.id
        ]

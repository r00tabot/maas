...
class TestSwitchesApi(ApiCommonTests):
    ...
    async def test_create_switch_with_image(
        ...
        """Test creating a new switch with ONIE image (full name)."""
        services_mock.switches = Mock(SwitchesService)
        services_mock.interfaces = Mock(InterfacesService)
        services_mock.interfaces.list.return_value = ListResult[Interface](
            items=[], total=0
    ...
    async def test_create_switch_with_nonexistent_image(
        ...
        """Test creating switch with non-existent image returns 422 for both full and short name formats."""
        services_mock.switches = Mock(SwitchesService)
        services_mock.interfaces = Mock(InterfacesService)
        services_mock.interfaces.list.return_value = ListResult[Interface](
            items=[], total=0
    ...
    async def test_update_switch_with_onie_image(
        self,
        services_mock: ServiceCollectionV3,
        mocked_api_client_admin: AsyncClient,
    ) -> None:
        """Test updating switch with ONIE image returns updated image information."""
        services_mock.switches = Mock(SwitchesService)
        services_mock.switches.get_by_id.return_value = TEST_SWITCH
    ...
    async def test_create_switch_with_onie_short_name(
        self,
        services_mock: ServiceCollectionV3,
        mocked_api_client_admin: AsyncClient,
    ) -> None:
        """Test creating switch with ONIE short name (vendor-version format)."""
        ...
        services_mock.boot_resources.get_one.side_effect = [None, onie_image]
        ...
        assert response.status_code == 201
        switch = SwitchResponse(**response.json())
        assert switch.target_image_id == 10

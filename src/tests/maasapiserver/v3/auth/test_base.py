...
def _build_auth_app(
    ...
    # We override the ``services`` dependency to inject the OpenFGA mock
    # after ServicesMiddleware populates ``request.state.services``.
    ...
    # Override the services dependency to inject the OpenFGA mock.
...
def auth_app(
    ...
    # The default app grants CAN_EDIT_GLOBAL_ENTITIES so that all
    # non-RBAC endpoints pass the OpenFGA check.

...
class AsyncOpenFGAClientMock:
    def __init__(
        ...
        visible_pools: list[int] | None = None,
        available_pools: list[int] | None = None,
        deployable_pools: list[int] | None = None,
        editable_pools: list[int] | None = None,
    ):
        self._permissions = permissions or []
        self._visible_pools = visible_pools or []
        self._available_pools = available_pools or []
        self._deployable_pools = deployable_pools or []
        self._editable_pools = editable_pools or []
    ...
    def __getattr__(self, item):
        ...
        if item == "list_pools_with_view_machines_access":

            async def _list_pools_with_view_machines_access(
                *args, **kwargs
            ) -> list[int]:
                return self._visible_pools

            return _list_pools_with_view_machines_access

        if item == "list_pools_with_available_machines_access":

            async def _list_pools_with_available_machines_access(
                *args, **kwargs
            ) -> list[int]:
                return self._available_pools

            return _list_pools_with_available_machines_access

        if item == "list_pools_with_deployable_machines_access":

            async def _list_pools_with_deployable_machines_access(
                *args, **kwargs
            ) -> list[int]:
                return self._deployable_pools

            return _list_pools_with_deployable_machines_access

        if item == "list_pools_with_editable_machines_access":

            async def _list_pools_with_editable_machines_access(
                *args, **kwargs
            ) -> list[int]:
                return self._editable_pools

            return _list_pools_with_editable_machines_access
...
def mocked_api_client_user_with_permissions(
    ...
    """Return a factory that grants the given OpenFGA permissions to the

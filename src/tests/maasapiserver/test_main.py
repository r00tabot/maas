...
async def server(db, ca_cert):
    ...
    app.get_app().add_api_route("/test", endpoint=index, methods=["GET"])
...
class TestMTLSInternalServer:
    ...
    @pytest.mark.asyncio
    async def test_mtls_without_client_certificate(self, server, ca_cert):
        async with httpx.AsyncClient(verify=str(ca_cert)) as client:
            r2 = await client.get("https://localhost:9443/test")
        assert r2.status_code == 200
        data2 = r2.json()
        tls_ext2 = data2["tls"]
        assert tls_ext2["tls_used"] is True
        assert tls_ext2["client_cert_chain"] == []  # none provided

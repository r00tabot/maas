...
class TestUbuntuProductList:
    def test_from_json(self):
        ubuntu_product_list = SimpleStreamsUbuntuProductList(**UBUNTU_PRODUCTS)
        assert len(ubuntu_product_list.products) == 1
        product = ubuntu_product_list.products[0]
        assert isinstance(product, UbuntuProduct)
        assert len(product.versions) == 2
        version = product.versions[0]
        assert isinstance(version, UbuntuImageVersion)
        files = version.get_downloadable_files()
        assert len(files) == 4
        for file in files:
            assert isinstance(file, UbuntuImageFile)


class TestCentosProductList:
    def test_from_json(self):
        centos_product_list = SimpleStreamsCentosProductList(**CENTOS_PRODUCTS)
        assert len(centos_product_list.products) == 1
        product = centos_product_list.products[0]
        assert isinstance(product, CentosProduct)
        assert len(product.versions) == 2
        version = product.versions[0]
        assert isinstance(version, CentosImageVersion)
        files = version.get_downloadable_files()
        assert len(files) == 2
        for file in files:
            assert isinstance(file, CentosImageFile)
...
class TestProductListFactory:
    def test_produce(self):
        ...
        assert isinstance(product_lists[1], SimpleStreamsUbuntuProductList)
        assert isinstance(product_lists[2], SimpleStreamsCentosProductList)

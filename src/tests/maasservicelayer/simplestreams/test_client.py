...
class TestSimpleStreamsClient:
    ...
    async def test_http_get_uses_proxy(self, mocker, mock_aioresponse) -> None:
        mocker.patch("os.path.exists").return_value = True
        url = "http://foo.com"
    ...
    async def test_http_get_raise_status(
    ...
    async def test_get_index_skip_verification(
        self, mocker, mock_aioresponse
    ) -> None:
        mocker.patch("os.path.exists").return_value = True
        url = "http://foo.com"
        mock_aioresponse.get(
            f"{url}/{UNSIGNED_INDEX_PATH}", payload=SAMPLE_INDEX
        )
        async with SimpleStreamsClient(

...
class TestBootResourceSetsService:
    ...
    async def test_pre_delete_hook_deletes_files(
        ...
        mock_boot_resource_files_service.delete_many.return_value = [
            TEST_BOOT_RESOURCE_FILE
        ]

        await service.delete_by_id(TEST_BOOT_RESOURCE_SET.id)

        mock_boot_resource_files_service.get_files_in_resource_set.assert_awaited_once_with(
            1
        )

        mock_boot_resource_files_service.delete_many.assert_awaited_once_with(
            query=QuerySpec(
                where=BootResourceFileClauseFactory.with_ids(
                    [TEST_BOOT_RESOURCE_FILE.id]
                )
            )
        )

    async def test_pre_delete_hook_no_files(
        self,
        mock_repository: Mock,
        mock_boot_resource_files_service: Mock,
        service: BootResourceSetsService,
        ...
        mock_boot_resource_files_service.get_files_in_resource_set.assert_called_once_with(
            1
        )

        mock_boot_resource_files_service.delete_many.assert_not_awaited()
    ...
    async def test_pre_delete_many_hook_deletes_files(
        ...
        mock_boot_resource_files_service.get_many.return_value = [
            TEST_BOOT_RESOURCE_FILE
        ]
        mock_boot_resource_files_service.delete_many.return_value = [
            TEST_BOOT_RESOURCE_FILE
        ...
        mock_boot_resource_files_service.get_many.assert_awaited_once_with(
            query=QuerySpec(
    ...
    async def test_pre_delete_many_hook_no_files(
        self,
        mock_repository: Mock,
        mock_boot_resource_files_service: Mock,
        service: BootResourceSetsService,

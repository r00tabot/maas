...
class TestImageSyncService:
    ...
    async def test_write_keyring_data_to_tmp_file(self, mocker) -> None:
        # return a mocked file to assert that data has been written
        mock_file = AsyncMock()
        mocker.patch(
            "aiofiles.tempfile._temporary_file"
        ).return_value = mock_file
        await self.service._write_keyring_data_to_tmp_file(b"abc123")
    ...
    async def test_fetch_images_metadata(self, mocker) -> None:
        ...
        aiofiles.os.unlink.assert_called_once_with(str(mock_file.name))
    ...
    async def test_delete_old_boot_resources__keep_image(self) -> None:
        ...
        self.service._boot_resource_is_selected = Mock(return_value=True)
        self.service._boot_resource_is_duplicated = Mock(return_value=False)
    ...
    async def test_delete_old_boot_resources__deletes_image(self) -> None:
        ...
        self.boot_resource_sets_service.get_latest_for_boot_resource.return_value = BOOT_RESOURCE_SET_ORACULAR
        self.service._boot_resource_is_selected = Mock(return_value=False)
        self.service._boot_resource_is_duplicated = Mock(return_value=False)

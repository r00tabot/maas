...
class TestBootResourceFileSyncRepository(
    ...
    async def test_get_synced_regions(
        self,
        repository_instance: BootResourceFileSyncRepository,
        fixture: Fixture,
    ) -> None:
        ...
        synced_regions_first_file = (
            await repository_instance.get_synced_regions({files[0].id})
        )
        ...
        synced_regions_second_file = (
            await repository_instance.get_synced_regions({files[1].id})
        )
        ...
        synced_regions_third_file = (
            await repository_instance.get_synced_regions({files[2].id})
        )

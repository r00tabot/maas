...
async def fga_server(tmp_path: Path):
    """Fixture to spin up a real aiohttp server on a Unix socket."""
...
async def fga_client(fga_server):
    """Fixture to initialize the client pointing to the real UDS server."""
...
class TestOpenFGAClient:
    ...
    async def test_check_returns_allowed_false(self, fga_client, fga_server):
        ...
        result = await fga_client.can_user_create_pools("user-1")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "can_user_create_pools",
                ("u1",),
                "pools.create",
                "system:system",
            ),
            ("can_user_delete_pool", ("u1", "p1"), "pool.delete", "pool:p1"),
            ("can_user_edit_pool", ("u1", "p1"), "pool.edit", "pool:p1"),
            (
                "can_user_add_machines_to_pool",
                ("u1", "p1"),
                "pool.machines.add",
                "pool:p1",
            ),
            (
                "can_user_remove_machines_from_pool",
                ("u1", "p1"),
                "pool.machines.remove",
                "pool:p1",
            ),
            (
                "can_user_view_machines_in_pool",
                ("u1", "p1"),
                "pool.machines.view",
                "pool:p1",
            ),
            (
                "can_user_deploy_machines_in_pool",
                ("u1", "p1"),
                "pool.machines.deploy",
                "pool:p1",
            ),
            (
                "can_user_manage_machines_in_pool",
                ("u1", "p1"),
                "pool.machines.manage",
                "pool:p1",
            ),
        ],
    )
    async def test_permission_methods(
        ...
        # Check payload sent to the server
    ...
    async def test_check_raises_for_status(self, fga_client, fga_server):
        ...
        with pytest.raises(httpx.HTTPStatusError):
            await fga_client.can_user_create_pools("u1")

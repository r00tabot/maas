...
from maascommon.osystem.onie import ONIEOS
...
class TestONIEOS:
    ...
    def test_parse_release_valid(
        ...
        onie = ONIEOS()
        result = onie.parse_release(release)
    ...
    def test_parse_release_invalid(self, release):
        onie = ONIEOS()
        assert onie.parse_release(release) is None

    def test_parse_release_normalizes_uppercase(self):
        onie = ONIEOS()
        result = onie.parse_release("VENDOR-1.0")
    ...
    def test_get_image_filetypes(self):
        ...
        # ONIE images use self-extracting installer type

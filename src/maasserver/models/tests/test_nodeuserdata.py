...
class TestNodeUserDataManager(MAASServerTestCase):
    ...
    def test_set_user_data_creates_new_nodeuserdata_if_needed(self):
        ...
        data = b"foo"
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, data)
        self.assertEqual(data, NodeUserData.objects.get(node=node).data)

    def test_set_user_data_overwrites_existing_userdata(self):
        node = factory.make_Node()
        data = b"bar"
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, b"old data")
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, data)
        self.assertEqual(data, NodeUserData.objects.get(node=node).data)

    def test_set_user_data_leaves_data_for_other_nodes_alone(self):
        node = factory.make_Node()
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, b"intact")
    ...
    def test_set_user_data_to_None_removes_user_data(self):
        node = factory.make_Node()
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, b"original")
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, None)
    ...
    def test_get_user_data_retrieves_data(self):
        ...
        NodeUserData.objects.set_user_data_for_ephimeral_env(node, data)
        self.assertEqual(
            data, NodeUserData.objects.get_user_data_for_ephimeral_env(node)
        )
    ...
    def test_get_user_data_raises_DoesNotExist_if_not_found(self):
        ...
        self.assertRaises(
            ...
            NodeUserData.objects.get_user_data_for_ephimeral_env,
            node,
        )
    ...
    def test_bulk_set_user_data(self):
        ...
        for node in nodes:
            self.assertEqual(
                data,
                NodeUserData.objects.get_user_data_for_ephimeral_env(node),
            )

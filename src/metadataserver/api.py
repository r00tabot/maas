...
class VersionIndexHandler(MetadataViewHandler):
    ...
    def _process_releasing(self, node, request, status):
        ...
        if status == SIGNAL_STATUS.OK:
            # Release scripts were executed successfully. The machine will be transitioned to the ready state only when
            # cloud-init sends the "modules-final" last event.
            Event.objects.create_node_event(
                node, EVENT_TYPES.NODE_RELEASE_SCRIPTS_OK
            )
        if status == SIGNAL_STATUS.FAILED:
    ...
    def read(self, request, version, mac=None):
        ...
        try:
            ...
            if node.get_boot_purpose() == "poweroff":
                ...
                # The machine has rebooted and asked for the user data. We have to serve the user data for the user environment.
                if node.status == NODE_STATUS.DEPLOYING:
                    user_data = (
...
class CurtinUserDataHandler(MetadataViewHandler):
    ...
    def read(self, request, version, mac=None):
        check_version(version)
        node = get_queried_node(request, for_mac=mac)
        user_data = NodeUserData.objects.get_user_data_for_ephimeral_env(node)
        user_data = prepare_user_data(user_data)
    ...
    def read(self, request, version, mac=None):
        ...
        with tarfile.open(mode="w", fileobj=binary) as tar:
            ...
            if (
                ...
                print(node.current_deployment_script_set)
                meta_data = self._add_script_set_to_tar(

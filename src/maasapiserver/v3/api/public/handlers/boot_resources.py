...
from maascommon.utils.images import (
    file_descriptor_or_path_to_path,
...
class BootResourcesHandler(Handler):
    ...
    def _create_hardlink(
        self, target_path: Path, source_path: StrOrBytesPath
    ) -> None:
    ...
    async def upload_boot_resource(
        ...
        async with NamedTemporaryFile(
            ...
            async for chunk in request.stream():
                sha256.update(chunk)
                bytes_written += await tmp_file.write(chunk)
            ...
            if not local_resource_file.path.exists():
                await asyncio.to_thread(
                    self._create_hardlink,
                    local_resource_file.path,
                    file_descriptor_or_path_to_path(tmp_filename),
                )
                logger.info(
            ...
            _ = await services.boot_resource_file_sync.get_or_create(
                query=QuerySpec(

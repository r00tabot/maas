...
class OpenFGATuplesRepository(Repository):
    ...
    async def create(self, builder: OpenFGATupleBuilder) -> OpenFGATuple:
        ...
        # Populate the store and inserted_at fields which are not present in the domain model. This is because the store is
        # always "MAAS" for tuples created via MAAS, and inserted_at is automatically set to the current time.

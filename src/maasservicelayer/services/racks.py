...
class RacksService(BaseService[Rack, RacksRepository, RackBuilder]):
    ...
    async def generate_bootstrap_token(self, resource: Rack) -> dict:
        ...
        secure communication between a running MAAS instance and a rack. In
        ...
        if cluster_cert_paths is not None:
            ...
            try:
                with (
                    ...
                    open(cert_path, "r") as cert_file,
                ):
                    key = key_file.read()
                    cert = cert_file.read()
                    cacerts = ""
                    try:
                        with open(cacerts_path, "r") as cacerts_file:
                            cacerts = cacerts_file.read()

...
class BootResourceSetsService(
    ...
    async def sync_progress(self, resource_set_id: int) -> float:
    ...
    async def sync_complete(self, resource_set_id: int) -> bool:
        sync_progress = await self.sync_progress(resource_set_id)
        return math.isclose(sync_progress, 100.0)

    async def commissionable(self, resource_set_id: int) -> bool:

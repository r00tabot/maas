...
class BaseEntitlementResourceBuilderFactory:
    ...
    def validate_entitlement(
        ...
        if entitlement_name not in cls.ENTITLEMENTS:
            return (
                False,
                f"Entitlement {entitlement_name} is not defined for pool resources. Valid entitlements are: {list(cls.ENTITLEMENTS.keys())}",
            )
        return True, None
...
class EntitlementsBuilderFactory:
    ...
    @classmethod
    def get_factory(
        cls, entitlement_name: str, resource_type: str

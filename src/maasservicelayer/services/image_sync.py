...
class ImageSyncServiceCache(ServiceCache):
    simplestreams_products: list[SimpleStreamsProductList] | None = None
...
class ImageSyncService(Service):
    ...
    async def get_files_to_download_from_product(
        ...
        for file in version.get_downloadable_files():
            ...
            if (
                local_file.complete
                and await self.boot_resource_files_service.sync_complete(
                    resource_file.id
                )
            ):
                logger.debug(
    ...
    async def get_longest_support_commissioning_boot_resource(
        self,
    ) -> BootResource | None:
        ...
        for boot_resource in await self.boot_resources_service.get_many(
            ...
            if (
                resource_set
                and await self.boot_resource_sets_service.sync_complete(
                    resource_set.id
                )
                and await self.boot_resource_sets_service.commissionable(
                    resource_set.id
                )
            ):
    ...
    async def set_global_default_releases(self):
        ...
        if not configs[CommissioningDistroSeriesConfig.name]:
            default_resource = (
                await self.get_longest_support_commissioning_boot_resource()
            )
            if default_resource:
                ...
                await self.configurations_service.set(
                    CommissioningDistroSeriesConfig.name, osystem
                )
        ...
        if not configs[DefaultDistroSeriesConfig.name]:
            default_resource = (
                default_resource
                or await self.get_longest_support_commissioning_boot_resource()
            )
            if default_resource:
                osystem, release = default_resource.name.split("/")
                await self.configurations_service.set(
        ...
        for boot_resource in boot_resources:
            ...
            for resource_set in resource_sets:
                if (
                    ...
                    or not await self.boot_resource_sets_service.sync_complete(
                        resource_set.id
                    )
                ):

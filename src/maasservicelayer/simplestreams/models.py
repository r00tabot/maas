...
This module contains the Pydantic models which represent SimpleStreams metadata.

The /streams/v1/index.json result is mapped as a `SimpleStreamsIndexList`.
...
- `SimpleStreamsUbuntuProductList`
- `SimpleStreamsCentosProductList`

The original structure of the SimpleStreams JSON has been slightly refactored
through the use of pydantic validators. See the preprocess_* validators.j
"""
...
class UbuntuImageFile(DownloadableFile):
    kpackage: str | None


class CentosImageFile(DownloadableFile):
    pass
...
class UbuntuImageVersion(Version):
    support_eol: datetime | None
    support_esm_eol: datetime | None
    boot_initrd: UbuntuImageFile = Field(..., alias="boot-initrd")
    boot_kernel: UbuntuImageFile = Field(..., alias="boot-kernel")
    manifest: UbuntuImageFile
    root_image_gz: UbuntuImageFile | None = Field(None, alias="root-image.gz")
    squashfs: UbuntuImageFile | None
...
class CentosImageVersion(Version):
    manifest: CentosImageFile
    root_tgz: CentosImageFile = Field(..., alias="root-tgz")
...
class UbuntuProduct(ImageProduct):
    kflavor: str
    krel: str  # seems to not be used
    release_codename: str
    versions: list[UbuntuImageVersion]

    @override
    @staticmethod
    def version_class() -> Type[Version]:
        return UbuntuImageVersion


class CentosProduct(ImageProduct):
    versions: list[CentosImageVersion]

    @override
    @staticmethod
    def version_class() -> Type[Version]:
        return CentosImageVersion
...
class SimpleStreamsUbuntuProductList(SimpleStreamsProductList):
    products: list[UbuntuProduct]

    @override
    @staticmethod
    def product_class() -> type[Product]:
        return UbuntuProduct


class SimpleStreamsCentosProductList(SimpleStreamsProductList):
    products: list[CentosProduct]

    @override
    @staticmethod
    def product_class() -> type[Product]:
        return CentosProduct
...
class SimpleStreamsProductListFactory:
    ...
    def produce(data) -> SimpleStreamsProductList:
        ...
        try:
            product = SimpleStreamsUbuntuProductList(**data)
        except ValidationError:
            pass

        try:
            product = SimpleStreamsCentosProductList(**data)
        except ValidationError:
            pass

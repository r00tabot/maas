...
type Allocator4 interface {
	...
	ACKLease(context.Context, net.IP, net.HardwareAddr) error
	NACKLease(context.Context, int, net.HardwareAddr) error
...
type dqliteAllocator4 struct {
	...
	hostname string  // attached to the allocator to only need to lookup once for all allocations
}
...
func (d *dqliteAllocator4) beginTx(ctx context.Context) (*sql.Tx, error) {
	if d.tx != nil {
		return d.tx, nil
	}
...
func (d *dqliteAllocator4) getHostReservationIfExists(ctx context.Context, tx *sql.Tx, vlanID int, mac net.HardwareAddr) (*HostReservation, error) {
	row := tx.QueryRowContext(ctx, getHostReservationForMAC, vlanID, strings.ToLower(mac.String()))
	hr := &HostReservation{}
...
func (d *dqliteAllocator4) ACKLease(ctx context.Context, ip net.IP, mac net.HardwareAddr) error {
	tx, err := d.beginTx(ctx)
	if err != nil {
		return err
	}
	...
	if lease.State != LeaseStateOffered {
		return fmt.Errorf("%w: lease is not in offered state", ErrInvalidDHCP4State)
	}
...
func (d *dqliteAllocator4) NACKLease(ctx context.Context, ifaceIdx int, mac net.HardwareAddr) error {
	tx, err := d.beginTx(ctx)
	if err != nil {
		return err
	}

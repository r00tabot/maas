...
func createPools(ctx context.Context, tx *sql.Tx) error {
	...
	for _, poolID := range poolIDs {
		insertStmt, insertArgs, err := builder.
			...
			Values(
				storeID, // must exist in scope
				"maas:0",
				"user",
				"parent",
				"pool",
				strconv.FormatInt(poolID, 10), // object_id as TEXT
				ulid.Make().String(),
				sq.Expr("NOW()"),
			).
			ToSql()
...
func Up00002(ctx context.Context, tx *sql.Tx) error {
	...
	relations := []string{"can_edit_machines", "can_edit_global_entities", "can_edit_controllers", "can_edit_permissions",
		"can_edit_configurations", "can_edit_notifications", "can_view_devices", "can_view_ipaddresses"}

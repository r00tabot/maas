...
func createStore(ctx context.Context, tx *sql.Tx) error {
	stmt, args, err := sq.StatementBuilder.PlaceholderFormat(sq.Dollar).
		...
		Values("00000000000000000000000000", "MAAS", sq.Expr("NOW()"), sq.Expr("NOW()")).
...
`
	...
	model.Id = "00000000000000000000000000"
	...
	stmt, args, err := sq.StatementBuilder.PlaceholderFormat(sq.Dollar).
		...
		Values("00000000000000000000000000", model.GetId(), model.GetSchemaVersion(), "", nil, pbdata).
		ToSql()
